...
def main():
    ...
    # Data extraction
    gt_values = [len(birds_data['points'][jpg_file]['bird']) for jpg_file in jpg_files if jpg_file in birds_data['points']]
    ...
    rmse_per_num_boxes = {num_box: np.sqrt(np.mean(np.array(differences_list) ** 2))
                          for num_box, differences_list in rmse_by_num_boxes.items()}
    
    # Plotting
    threshold = 1
    num_boxes_filtered, rmse_filtered = zip(*[(num_box, rmse) for num_box, rmse in rmse_per_num_boxes.items() if num_box >= threshold])
