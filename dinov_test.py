...
def process_and_save_results(model, images, reference_images, results_path='dino-results'):
    ...
    for i, (img, image_path) in enumerate(images):
        ...
        save_path = os.path.join(results_path, 'drawn', f'{i}.jpg')
        os.makedirs(os.path.join(results_path,'drawn'), exist_ok=True)
...
def main():
    ...
    os.makedirs("dino-results", exist_ok=True)
    os.makedirs(os.path.join("dino-results", "ref-images", exist_ok=True))
    ...
    for image_name, annotations in data.items():
        if len(annotations['bird']) > 6:
            ...
            with Image.open(image_path) as img:
                ...
                img_with_overlay.save(os.path.join("dino-results", "ref-images", image_name))
    ...
    process_and_save_results(model, inference_images, reference_images)
